...
class GamePagePresenter: MvpPresenter<GamePageView>() {
    var score: Int = 0
    ...
    private fun boostScore() {
        score += 10
        if (score > preferences?.getHighScore() as Int) preferences?.saveHighScore(
            score
        )
    }
            ...
            if (!moveValid(coordinate as Point, frameNumber)) {
                ...
                if (Motions.DOWN.name == action) {
                    ...
                    if (!blockAdditionPossible()) {
                        currentState = Statuses.OVER.name; currentBlock = null;
                        resetField(false); }
                }
            } else {
                if (frameNumber != null) {
    ...
    private fun assessField() {
        for (i in field.indices) {
            ...
            for (j in field[i].indices) {
                ...
                val isEmpty =
                    CellConstants.EMPTY.value == status
    ...
    enum class CellConstants(val value: Byte) { EMPTY(0), EPHEMERAL(1) }
    enum class Statuses { AWAITING_START, ACTIVE, INACTIVE, OVER }
    enum class Motions { LEFT, RIGHT, DOWN, ROTATE }
}